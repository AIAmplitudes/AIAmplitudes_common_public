"""
Parsers for symbol data files in Maple-like custom notation.

Symbol files store coefficients as signed integers multiplied by SB(...) or E(...)
basis elements. The convert() function is the main entry point, producing a
dict of {key_string: integer_coefficient} from the raw file format.

Supported formats:
  - Full: SB(abcdef) notation, one term per symbol entry
  - Quad: Esymbquad[L] blocks, keys end with @QUAD_i tokens
  - Oct: Esymboct[L] blocks, keys end with @OCT_i tokens

Also provides readers for front/back space files (FBconvert), relation files
(read_rels_perm), and generic prefix-delimited blocks (readFile).
"""
...
def convert(filename, loop=None, reptype=None):
    """Parse a symbol data file into a {key: coefficient} dict.

    Args:
        filename: Path to the symbol file.
        loop: Loop order L (determines which block to read).
        reptype: 'quad', 'oct', or None for full format.
    """
...
def readSymb(filename, prefix, loop=None):
    """Read a named symbol block from a file. Returns the raw string content."""
...
def readFile(f, prefix):
    """Read a prefix-delimited block from an open file handle.

    Scans for a line starting with `prefix`, then accumulates lines
    (handling backslash continuations) until a blank line or terminator.
    """
    res = ''
    reading_form = False
    for line in f:
...
def SB_to_dict(mystring):
    """Parse a string of SB(...) terms into a {key: coefficient} dict."""
    def to_coef(mystr):
        if mystr == '-':
            return -1
        elif mystr == '':
...
def read_rels_perm(mydir, weight, seam="front", reltype="oneletter"):
    """Read pre-generated permissive coproduct relations from Fspace_rels/Bspace_rels files.

    These are the sewing checks generated by Lance & Garrett's seam_sewing_rels.py,
    stored as sewrelsf[w] / sewrelsb[w] blocks.
    """
...
def readcrel(crel, w=2, seam="back"):
    """Parse a single coefficient-form relation string like '+2*c[a,3]-c[b,1]=0'
    into a dict mapping sewing keys (e.g. 'a@BP_2_3') to integer/Fraction coefficients."""
...
def get_relpermdict(mydir, w, seam, reltype):
    """Load permissive relations as a list of {sewing_key: coefficient} dicts."""
